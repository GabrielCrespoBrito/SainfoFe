...
function registrarCliente(nombre_documento) {
  ...
  let funcs = {
    ...
    success: function (info) {
      console.log("success creacion del cliente", info);
      //
      ...
      if ($cliente_documento.length) {
        if (!$cliente_documento.val()) {
          ...
          // console.log("tipodocumento", info.TDocCodi, tipodocumento);
...
$(document).ready(function (e) {
  ...
  function calculosItem(data) {
    ...
    // info.total = Number(fixedNumber(info.valorVentaPorItem + info.igv + info.valorISCTotal));
  ...
  function poner_totales_cant() {
    console.log(
      "cargo_global poner_totales_cant",
      $("[name=cargo_global]").val()
    );
  ...
  function agregar_item() {
    ...
    if (action_i() == "create") {
      console.log("current_Data", current_product_data);
  ...
  function add_item(info, fromForm = false, cotizacion = false, cleanItems = false) {
    show_comment_div();
    hide_comment();
  ...
  function dcto_defecto(data) {
    ...
    // console.log("poner_producto descuento" , data );
  }
  ...
  function poner_data_producto(data) {
    ...
    console.log("poniend data pcurrent_product_data", data);
  ...
  function verificar_data_factura() {
    ...
    var tipo_documento = $(".box-body [name=tipo_documento] option:selected");
    ...
    if (tipo_guia_asoc == "asociar") {
      ...
      for (let i = 0; i < $select_guias.length; i++) {
        ...
        console.log("numero de guia", guia_id, $select_guias[i])
    ...
    if (tipodocumentoCode == '07' || tipodocumentoCode == '08') {
      ...
      // console.log("$('[name=ref_tipo]') == null", $("[name=ref_tipo]") == null);      
    ...
    if (fp.data('credit')) {
      ...
      let total_documento = Number($("[name=total_importe]").val());
      ...
      if (total_fp != total_documento) {
        notiYFocus("cliente_documento", "El fraccionamiento de pagos no coincide con el total del documento");
        $("#modalFP").modal('show');
        return;
      }
    }
  ...
  function guardar_factura(data) {
    ...
    if (data.imprecion_data.impresion_directa) {
      ...
      function impresionError(data) {
        console.log("impresionError", data);
        successStore(true);
      }
  ...
  function successStore(show_window_print = true) {
    ...
    if (show_window_print) {
      window.open(window.data_guardado.url, "_blank", "toolbar=yes,scrollbars=yes");
    }
  ...
  function aceptar_guardado() {
    // loremp-ipsum-odlor-qwerty
    console.log("executing_ajax", window.executing_ajax);
    // executing_ajax|executing_ajax|executing_ajax|executing_ajax|executing_ajax|executing_ajax|executing_ajax
    if (window.executing_ajax) {
      return false;
    }
    let items = [];
  ...
  function select2_init() {
    ...
    // // Buscar Guia
    // let url_search_guia = $('.select-guia-select2').attr('data-url');

    // $('.select-guia-select2').select2({
  ...
  function editProductoToTable($tr) {
    ...
    // console.log( "tr edit codigo de barra", info, calculo_result );
  ...
  function cargoGlobalInput() {
    ...
    console.log({ tipo_cargo_value, cargo_value });
  ...
  function events() {
    ...
    onScan.attachTo(document, {
      ...
      onKeyDetect: function (iKeyCode) { // output all potentially relevant key events - great for debugging!
        // removeFocusInElement();
        // console.log("onKeyDetect", iKeyCode)
  ...
  function initDatatables() {
    ...
    function dataUnidadPrincipal(value, type, data, settings) {
      // console.log("UnidadPrincipal", data );
