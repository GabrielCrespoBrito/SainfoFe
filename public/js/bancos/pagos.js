let AppPago = 
{
  ...
  set_urls : function(){
    this.urls = {
      ...
      update: $(this.parent).data('urlsave'),
  ...
  setTitle(action)
  {
    this.eles.title.text(this.data.messageTitle[action]);
    let showCorrelativePago =  action == "create";
    this.eles.title_correlative.toggleClass( 'hide' , showCorrelativePago);    
  },

  defaulState : function()
  {
  ...
  getUrl : function()
  {
    ...
    // @TODO Poner la url de modificación 
    else {
...
let AppPagosIndex = {
  ...
  set_urls : function(){  
    this.urls = {
      pagos  : $( '.botones_div' , this.parent ).data('urlpagos'),
      remove : $( '.botones_div' , this.parent ).data('urlremove')
    }
  },
  ...
  successRemove : function(importeporpagar){

    this.eles.saldo.text(importeporpagar);
    this.current_tr.hide( 300 ,function(){
      $(this).remove()
    });
