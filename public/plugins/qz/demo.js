  ...
  qz.security.setCertificatePromise(function(resolve, reject) {
    ...
    // Route:: get('/', 'TestsController@firmaDigitalImpresion');

    let url = $("#enlaces").attr('data-href_publica');

    fetch( url , { cache: 'no-store', headers: { 'Content-Type': 'text/plain' } })
      .then(function (data) { console.log("setCertificatePromise" , data )  ;data.ok ? resolve(data.text()) : reject(data.text()); });
  ...
  qz.security.setSignaturePromise(function(toSign) {
    ...
    //  .then(function(data) { data.ok ? resolve(data.text()) : reject(data.text()); });
      ...
      let url = $("#enlaces").attr('data-href_privada');

    // Preferred method - from server
      fetch(`${url}?request=${toSign}`, {cache: 'no-store', headers: {'Content-Type': 'text/plain'}})
        .then(function (data) { console.log("setSignaturePromise", data); data.ok ? resolve(data.text()) : reject(data.text()); });
