...
<p>
    <a href="{{ route('admin.comandos', 'permisos') }}" class="btn btn-primary"> Registrar Nuevos Permisos Solo a Usuario
        Principal</a>
    <a href="{{ route('admin.comandos', 'permisos_all') }}" class="btn btn-primary"> Registrar Nuevos Permisos A todos los
        Usuarios</a>

    <a href="{{ route('admin.comandos', 'permisos_all_no_assign') }}" class="btn btn-primary"> Registrar Nuevos Permisos
        (Sin Asignar a ningun usuario)</a>

</p>

<p>
    <a href="{{ route('admin.comandos', 'medios_pagos') }}" class="btn btn-primary"> Sincronizar Medios de Pago </a>
    Registros A Empresas las Nuevas formas de pagos registradas en el sistema
</p>
...
<form action="{{ route('admin.save_img_footer_banner') }}" method="post" enctype="multipart/form-data">
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <input class="form-control input-sm {{ $errors->has('img_footer') ? 'has-error' : '' }}" require="required"
                name="img_footer" type="file">
        </div>
...
<form action="{{ route('admin.config.exeCode') }}" method="post">
    ...
    <div class="row">
        ...
        <div class="col-md-9">
            <input class="form-control input-sm {{ $errors->has('codigo') ? 'has-error' : '' }}" require="required"
