...
if (isset($locales)) {
    foreach ($locales as $local) {
        $data = [
            'class_name' => 'almacenes ' . $local->defecto ? 'local-principal' : '',
            'attributes' => [
...
@component('components.modal', ['id' => $id, 'title' => $title_modal])
    ...
    @slot('body')
        <div class="row botones_div">
            ...
            <div class="form-group col-md-4 p-0">
                <div class="form-group">
                    <select name="grupo_filter" data-url="{{ route('productos.buscar_grupo') }}" required="required"
                        class="form-control">
                        <option data-familias="" value=""> -- SELECCIONAR GRUPO -- </option>
                        @foreach ($grupos as $grupo)
                            <option data-familias="{{ $loop->first ? $grupo->familias() : '' }}" value="{{ $grupo->GruCodi }}">
                                {{ $grupo->GruNomb }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            @if ($nuevo_producto)
                <div class="col-md-1">
                    <a target="_blank" class="btn pull-right btn-primary btn-flat" href="{{ route('productos.index') }}"> <span
        ...
        @component('components.table', [
            ...
            'class_name' => 'sainfo table-oneline',
            'container_class' => 'productos_select',
