    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="row">
              ...
              <span class="input-group-addon"> Destinatario</span>
                <div class="fixed_position">
                 ...
                 <select id="destinatario" 
                    data-text="{{ $destinatario->getNombre() }}"
                    data-id="{{ $destinatario->getId() }}" 
                    name="destinatario"                    
          ...
          <div class="form-group col-md-12">  
            <div class="input-group">
              <span class="input-group-addon">Dirección Partida</span>
                <input class="form-control input-sm" name="direccion_partida" type="text" value="{{ $guia->guidirp ? $guia->guidirp : optional($guia->almacen)->LocDire }}">     
            </div>
          </div>
          ...
          <div class="form-group col-md-12">  
            <div class="input-group">
              <span class="input-group-addon">Dirección Llegada</span>
                <input class="form-control input-sm" name="direccion_llegada" type="text" value="{{ $guia->guidill ?? $guia->cliente->PCDire  }}">
            </div>
          </div>
          <div class="form-group col-md-12">  
           <div class="input-group">
            ...
            <div class="input-group">
              <span class="input-group-addon">Motivo Traslado</span>
                ...
                @foreach( cacheHelper('motivotraslado.all') as $motivo )  
                  <option data-type="{{ (int) $motivo->isProveedor() }}"  {{ $motivo->MotCodi == $guia->motcodi ? 'selected' : '' }}  value="{{ $motivo->MotCodi }}"> {{ $motivo->MotNomb }} </option>
                @endforeach
              </select>
            </div>
          </div>
            ...
            <div class="input-group">
              <span class="input-group-addon">Transportista</span>
                ...
                <select 
                  ...
                  data-text="{{  $transportistaNombre }}"
                  data-id="{{ $transportistaId }}" 
                  name="transportista"
                  style="display:none;position:absolute">
              </select>                
          ...
          <div data-field-traslado="02"  class="mod-campo form-group col-md-12">  
            <div class="input-group">
              ...
              <select 
                data-event="select2" 
                placeholder="Elegir Elegir Vehiculo" 
                data-minimuminputlength="0" 
                class="form-control input-sm select2" 
                data-url="{{ route('vehiculo.search') }}" 
                data-text="{{  $vehiculoNombre }}"
                data-id="{{ $vehiculoId }}" 
                name="placa"
                style="display:none;position:absolute">
              </select>  
          ...
          <div data-field-traslado="01" class="mod-campo form-group col-md-12">  
            <div class="input-group">
              <span class="input-group-addon">Empresa de Transporte</span>
                ...
                <select 
                  data-event="select2" 
                  placeholder="Elegir Empresa de transporte" 
                  data-minimuminputlength="0" 
                  class="form-control input-sm select2" 
                  data-url="{{ route('empresa_transporte.search') }}" 
                  data-text="{{  $empresaTransporteNombre }}"
                  data-id="{{ $empresaTransporteId }}" 
                  name="empresa"
                  style="display:none;position:absolute">
              </select> 
                <span class="input-group-addon" style="margin-bottom: 0">
                  <a href="{{ route('empresa_transporte.create') }}"  target="_blank" class="" id="notCredito">
                    <span class="fa fa-plus"></span>
                  </a>
                </span>                 
            </div>
          ...
          <div class="form-group col-md-6">  
            <div class="input-group">
              <span class="input-group-addon">Doc. Referencia</span>
                <input class="form-control input-sm" name="documento_referencia" disabled type="text" value="{{ $guia->docrefe }}">     
            </div>
          </div>
          <div class="form-group col-md-6">  
            <div class="input-group">
              <span class="input-group-addon"> Peso total</span>
                <input class="form-control input-sm" name="peso_total" type="text" min="0" value="{{ $guia->guiporp ?? 1 }}">     
            </div>
          </div>          
              ...
              <span class="input-group-addon">Serie</span>
                <select name="serie_documento" class="form-control input-sm">
                  @foreach($tipos_documentos as $tipo_documento)
                    <option {{ $loop->first ? 'selected' : '' }}   data-codigo="{{ $tipo_documento['nuevo_codigo'] }}" value="{{ $tipo_documento['id'] }}"> {{ $tipo_documento['nombre'] }} </option>
                  @endforeach
                </select>   
         ...
         <div class="form-group col-md-6">  
            <div class="input-group">
              <span class="input-group-addon"> Serie </span>
                <input class="form-control input-sm" name="serie_documento" type="text" value="{{ $guia->GuiSeri }}">     
            </div>
          </div>
          <div class="form-group col-md-6">  
            <div class="input-group">
              <span class="input-group-addon"> Número </span>
                <input class="form-control input-sm" name="numero_documento" type="text" value="{{ $guia->GuiNumee }}">     
            </div>
          </div>
          @endif
          </form>
        ...
        <div class="row">
          ...
          <label class="pull-right" for="imprimir_guia">  <input id="imprimir_guia" type="checkbox" name="imprimir"> Imprimir </label>
          @endif     
          </div>
        </div>
        @endif
