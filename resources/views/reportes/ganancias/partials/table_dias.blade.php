    ...
    @slot('body')
        @foreach ($data['items'] as $fecha => $data_fecha)
            ...
            <tr class="tr-dias {{ $data_fecha['info']['positive'] ? 'is-positive' : 'is-negative' }}">
                ...
                @if ($tableInHtml)
                    <td class="text-right">
                        <a target="_blank" href="#" class="btn btn-default btn-xs btn-flat span-info"> <span
                                class="fa fa-eye"></span></a>
                        <a target="_blank"
                            href="{{ route('reportes.utilidades.pdf_fecha', ['fecha' => $fecha, 'local' => $local, 'grupo' => $grupo, 'vendedor' => $vendedor, 'descontarPorcVendedor' => (int) $descontarPorcVendedor, 'zona' => $zona]) }}"
                            class="btn btn-default btn-xs btn-flat"> <span class="fa fa-file-pdf-o"
                                title="Reporte de los documentos del dia"></span></a>
                    </td>
                @endif
            </tr>
            ...
            @if ($tableInHtml)
                <tr class="tr-ventas tr-container-info" style="display:none">
                    <td colspan="{{ $colspan }}" class="pl-0 pr-0 text-right"
                        style="padding-left:0!important;padding-right:0!important">
                        @include('reportes.ganancias.partials.table_venta', [
                            'data' => $data_fecha,
                            'tableInHtml' => $tableInHtml,
                        ])
                    </td>
                </tr>
            @endif
