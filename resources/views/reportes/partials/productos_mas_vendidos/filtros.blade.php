...
<div class="filtros">
    <form action="{{ route('reportes.productos_mas_vendidos.pdf') }}" method="post">
        ...
        @if ($zona)
            @include('reportes.partials.general.zonas')
        @endif

        @if ($descontarPorcVendedor)
            <div class="row">
                <div class="col-md-12 pl-0" style="margin-bottom:20px">
                    <label style="float:right; font-weight:normal"> <input type="checkbox" name="descontar_porc_vendedor"
                            value="1"> Descontar Porc. Vendedor </label>
                </div>
            </div>
        @endif
