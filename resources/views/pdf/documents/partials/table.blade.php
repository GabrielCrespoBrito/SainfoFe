        ...
        <thead>
            <tr class="tr_head">
                @foreach ($theads as $td)
                    <td class="{{ $td['class_name'] }} {{ $thead_class }}" width="{{ $td['width'] }}">
                        {{ $td['text'] }} </td>
                @endforeach
        </thead>
        ...
        <tbody>
            ...
            @if ($venta2->hasAnticipo())
                ...
                <tr>
                    ...
                    <td class="{{ $class_precio_unit }} {{ $tbody_class }} ">
                        {{ convertNegativeStr(decimal($item->totales[$precioValorCampo])) }} </td>
                    <td class="{{ $class_importe }} {{ $tbody_class }} ">
                        {{ convertNegativeStr(decimal($item->getTotal($totalCampo))) }}</td>
                </tr>
            @endif

            @foreach ($items as $item)
                <tr>
                    <td class="{{ $class_orden }} {{ $tbody_class }} ">{{ addCero($item->DetItem + $item_add) }}
                    </td>
                    ...
                    <td class="{{ $class_precio_unit }} {{ $tbody_class }} ">
                        {{ decimal($item->getTotal($precioValorCampo), $decimals) }} </td>
                    <td class="{{ $class_importe }} {{ $tbody_class }} ">{{ decimal($item->getTotal($totalCampo)) }}
                    </td>
                </tr>
            @endforeach
