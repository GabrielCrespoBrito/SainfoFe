@php
    ...
    $total = $is_credito
        ? $creditos->sum('monto')
        : $venta2->VtaImpo - ($venta2->retencionMonto() + $venta2->detraccionMonto());
@endphp

<div class="{{ $class_name }}">
    <div class="titulo_div_class {{ $titulo_div_class }}">
        @if ($is_credito || $venta2->hasDetraccion() || $venta2->hasMontoRetencion())
            Monto neto Pendiente de pago:<span class="total-pendiente-pago"> {{ $moneda_abreviatura }}
                {{ fixedValue($total, $decimals) }} </span>
        @else
            ...
            @if ($is_credito)
                @foreach ($creditos as $credito)
                    <tr style="text-align: left;">
                        ...
                        <td class="{{ $tr_valor_class }}"> {{ $credito->getMonedaAbreviatura() }}
                            {{ $credito->monto }} </td>
                    </tr>
                @endforeach
            @else
                @foreach ($condiciones as $condicion)
                    <tr style="text-align:{{ $textAlign }};">
                        <td style="font-size:.7em;text-align:{{ $textAlign }}" class="pl-x4 {{ $tr_valor_class }}">
                            {{ $condicion }} </td>
                    </tr>
                @endforeach
            @endif
            ...
            @if (empcodi() == '008' && ($is_credito || $venta2->hasDetraccion() || $venta2->hasMontoRetencion()))
                <tr>
                    <td colspan="3">
                        ...
                        <div
                            style="font-weight:bold; padding-left:5px;border-top: 1px solid #000; border-bottom: 1px solid #000">
                            Condiciones de venta
                        </div>
                ...
                @foreach ($condiciones as $condicion)
                    <tr style="text-align:{{ $textAlign }};">
                        <td style="font-size:.7em; letter-spacing:2px; text-align:{{ $textAlign }}" class="pl-x4">
                            {{ $condicion }} </td>
                    </tr>
                @endforeach
            @endif
