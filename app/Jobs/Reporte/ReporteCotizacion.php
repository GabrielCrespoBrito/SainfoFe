...
class ReporteCotizacion
{
    ...
    public function getQuery()
    {
        $query = Cotizacion::with([
            ...
            ->when($this->usucodi, function ($query) {
    ...
    public function getInfoReporte()
    {
        $vendedor = $this->vendedor
            ? Vendedor::find($this->vendedor)->vennomb
            : "TODOS";
        $usuario = $this->usucodi
            ? User::find($this->usucodi)->usulogi
            : "TODOS";
        $estado = $this->estado
            ? ["P" => "Pendiente", "L" => "Liberada"][$this->estado]
            : "TODOS";
        $cotizacionzona = $this->zona
            ? Zona::find($this->zona)->ZonNomb
            : "TODOS";
        $empresa = get_empresa();
